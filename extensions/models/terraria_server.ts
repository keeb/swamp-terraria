/**
 * Model `@keeb/terraria/server` — controls a Terraria (tModLoader) server
 * running in a Docker container with tmux on a remote host. Supports broadcast
 * shutdown warnings, tmux-pane player-list parsing for status, and Prometheus
 * textfile metrics collection.
 */
...
const GlobalArgs = z.object({
  sshHost: z.string().nullable().describe(
    "SSH hostname/IP (set via CEL from lookup model)",
  ),
  sshUser: z.string().default("root").describe("SSH user (default 'root')"),
  containerName: z.string().default("tmodloader").describe(
    "Docker container name running tModLoader",
  ),
  serverName: z.string().default("server").describe(
    "Resource instance name for writeResource",
  ),
});
...
/** Terraria server control model definition. */
export const model = {
  type: "@keeb/terraria/server",
  ...
  methods: {
    warnShutdown: {
      ...
      execute: async (_args, context) => {
        const { sshHost, sshUser = "root", containerName = "tmodloader" } =
          context.globalArgs;
        ...
        const containerCheck = await sshExecRaw(
          sshHost,
          sshUser,
          `docker inspect --format '{{.State.Running}}' ${containerName} 2>/dev/null`,
        );
        if (
          containerCheck.code !== 0 || containerCheck.stdout.trim() !== "true"
        ) {
          console.log(
            `[warnShutdown] Container ${containerName} not running - skipping warning`,
          );
          const handle = await context.writeResource("server", "server", {
            success: true,
            skipped: true,
        ...
        const tmuxCheck = await sshExecRaw(
          sshHost,
          sshUser,
          `docker exec ${containerName} tmux has-session 2>/dev/null && echo exists || echo missing`,
        );
        if (tmuxCheck.stdout.trim() !== "exists") {
          console.log(
            `[warnShutdown] No tmux session in container - skipping warning`,
          );
          const handle = await context.writeResource("server", "server", {
            success: true,
            skipped: true,
        ...
        for (let i = 0; i < 3; i++) {
          await sshExecRaw(
            sshHost,
            sshUser,
            `docker exec ${containerName} tmux send-keys "say == SERVER SHUTTING DOWN IN 30 SECONDS ==" Enter`,
          );
        }
    ...
    status: {
      ...
      execute: async (_args, context) => {
        ...
        if (reachable.code !== 0) {
          console.log(
            `[status] SSH unreachable at ${sshHost} - VM may be stopped`,
          );
          const handle = await context.writeResource("server", "server", {
            serverRunning: false,
            timestamp: new Date().toISOString(),
        ...
        if (tmuxCheck.stdout.trim() !== "exists") {
          console.log(
            `[status] No tmux session in container - server not running`,
          );
          const handle = await context.writeResource("server", "server", {
            serverRunning: false,
            timestamp: new Date().toISOString(),
        ...
        await sshExecRaw(
          sshHost,
          sshUser,
          `docker exec ${containerName} tmux send-keys "playing" Enter`,
        );
        ...
        const paneResult = await sshExecRaw(
          sshHost,
          sshUser,
          `docker exec ${containerName} tmux capture-pane -p`,
        );
        ...
        if (countLineIdx >= 0) {
          ...
          const players = [];
          for (
            let i = countLineIdx - 1;
            i >= 0 && players.length < online;
            i--
          ) {
          ...
          console.log(
            `[status] ${online} player(s) connected: ${
              players.join(", ") || "(none)"
            }`,
          );
          const handle = await context.writeResource("server", "server", {
    ...
    collectMetrics: {
      ...
      execute: async (_args, context) => {
        ...
        if (
          ...
          console.log(
            `[collectMetrics] Container ${containerName} not running`,
          );
          const data = {
            serverRunning: false,
            online: 0,
        ...
        console.log(
          `[collectMetrics] ${online} player(s): ${
            players.join(", ") || "(none)"
          }`,
        );
